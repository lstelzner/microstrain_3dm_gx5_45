...
int main(int argc, char *argv[]) {
  ...
  if (mip_interface_init(com_port, baudrate, &device_interface,
  ...
  if (com_mode == MIP_SDK_GX4_45_IMU_DIRECT_MODE) {
    ...
    while (mip_base_cmd_get_device_supported_descriptors(
               &device_interface, (u8 *)device_descriptors,
    ...
    while (mip_base_cmd_built_in_test(&device_interface, &bit_result) !=
           MIP_INTERFACE_OK) {
    }
    ...
    while (mip_3dm_cmd_get_ahrs_base_rate(&device_interface, &base_rate) !=
           MIP_INTERFACE_OK) {
    }
    ...
    while (mip_3dm_cmd_hw_specific_imu_device_status(
               &device_interface, GX4_IMU_MODEL_NUMBER,
               GX4_IMU_BASIC_STATUS_SEL,
    ...
    while (mip_3dm_cmd_coning_sculling_compensation(
               &device_interface, MIP_FUNCTION_SELECTOR_WRITE, &enable) !=
           MIP_INTERFACE_OK) {
    }
    ...
    while (mip_3dm_cmd_coning_sculling_compensation(
               &device_interface, MIP_FUNCTION_SELECTOR_READ, &enable) !=
           MIP_INTERFACE_OK) {
    }

    printf("%s\n\n", enable == MIP_3DM_CONING_AND_SCULLING_DISABLE ? "DISABLED"
                                                                   : "ENABLED");
    ...
    while (mip_3dm_cmd_accel_bias(&device_interface,
    ...
    while (mip_3dm_cmd_accel_bias(&device_interface, MIP_FUNCTION_SELECTOR_READ,
                                  bias_vector) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_3dm_cmd_accel_bias(&device_interface,
                                  MIP_FUNCTION_SELECTOR_LOAD_DEFAULT,
    ...
    while (mip_3dm_cmd_gyro_bias(&device_interface, MIP_FUNCTION_SELECTOR_READ,
                                 bias_vector) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_3dm_cmd_gyro_bias(&device_interface,
                                 MIP_FUNCTION_SELECTOR_LOAD_DEFAULT,
                                 NULL) != MIP_INTERFACE_OK) {
    }
    ...
    printf(
        ...
        "\n");
    printf(
        "Performing Gyro Bias capture.\nPlease keep device stationary during "
        "the 5 second gyro bias capture interval\n");
    printf(
        "----------------------------------------------------------------------"
        "\n\n");
    ...
    while (mip_3dm_cmd_hard_iron(&device_interface, MIP_FUNCTION_SELECTOR_WRITE,
                                 hard_iron) != MIP_INTERFACE_OK) {
    }

    // Read back the hard iron offset values
    while (mip_3dm_cmd_hard_iron(&device_interface, MIP_FUNCTION_SELECTOR_READ,
    ...
    if ((abs(hard_iron_readback[0] - hard_iron[0]) < 0.001) &&
      ...
      printf("Sent values:     %f X %f Y %f Z\n", hard_iron[0], hard_iron[1],
             hard_iron[2]);
      printf("Returned values: %f X %f Y %f Z\n", hard_iron_readback[0],
             hard_iron_readback[1], hard_iron_readback[2]);
    }

    printf("\n\nLoading the default hard iron offset values.\n\n");

    while (mip_3dm_cmd_hard_iron(&device_interface,
                                 MIP_FUNCTION_SELECTOR_LOAD_DEFAULT,
                                 NULL) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_3dm_cmd_soft_iron(&device_interface, MIP_FUNCTION_SELECTOR_WRITE,
                                 soft_iron) != MIP_INTERFACE_OK) {
    }

    // Read back the soft iron matrix values
    while (mip_3dm_cmd_soft_iron(&device_interface, MIP_FUNCTION_SELECTOR_READ,
    ...
    if ((abs(soft_iron_readback[0] - soft_iron[0]) < 0.001) &&
      ...
      printf("Sent values:     [%f  %f  %f][%f  %f  %f][%f  %f  %f]\n",
             soft_iron[0], soft_iron[1], soft_iron[2], soft_iron[3],
             soft_iron[4], soft_iron[5], soft_iron[6], soft_iron[7],
             soft_iron[8]);
      printf(
    ...
    while (mip_3dm_cmd_soft_iron(&device_interface,
                                 MIP_FUNCTION_SELECTOR_LOAD_DEFAULT,
                                 NULL) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_3dm_cmd_ahrs_message_format(
               &device_interface, MIP_FUNCTION_SELECTOR_WRITE,
    ...
    while (mip_3dm_cmd_poll_ahrs(
               &device_interface, MIP_3DM_POLLING_ENABLE_ACK_NACK,
  ...
  if (com_mode == MIP_SDK_GX4_45_IMU_STANDARD_MODE) {
    ...
    while (mip_3dm_cmd_get_gps_base_rate(&device_interface, &base_rate) !=
           MIP_INTERFACE_OK) {
    }
    ...
    while (mip_3dm_cmd_complementary_filter_settings(
               &device_interface, MIP_FUNCTION_SELECTOR_WRITE,
    ...
    while (mip_3dm_cmd_complementary_filter_settings(
               &device_interface, MIP_FUNCTION_SELECTOR_READ,
               &comp_filter_readback) != MIP_INTERFACE_OK) {
    }

    if ((comp_filter_command.north_compensation_enable ==
         comp_filter_readback.north_compensation_enable) &&
        (comp_filter_command.up_compensation_enable ==
         comp_filter_readback.up_compensation_enable) &&
      ...
      printf(
          "Sent values:     Up Enable: %d North Enable: %d Up Time Constant: "
          "%f North Time Constant: %f \n",
          comp_filter_command.up_compensation_enable,
          comp_filter_command.north_compensation_enable,
          comp_filter_command.up_compensation_time_constant,
          comp_filter_command.north_compensation_time_constant);
      printf(
          "Returned values: Up Enable: %d North Enable: %d Up Time Constant: "
          "%f North Time Constant: %f \n",
          comp_filter_readback.up_compensation_enable,
          comp_filter_readback.north_compensation_enable,
          comp_filter_readback.up_compensation_time_constant,
          comp_filter_readback.north_compensation_time_constant);
    }

    printf("\n\nLoading the default complementary filter values.\n\n");

    while (mip_3dm_cmd_complementary_filter_settings(
               &device_interface, MIP_FUNCTION_SELECTOR_LOAD_DEFAULT, NULL) !=
           MIP_INTERFACE_OK) {
    }
    ...
    // Request basic status report
    while (mip_3dm_cmd_hw_specific_device_status(
               &device_interface, GX4_45_MODEL_NUMBER, GX4_45_BASIC_STATUS_SEL,
    ...
    // Request diagnostic status report
    while (mip_3dm_cmd_hw_specific_device_status(
               &device_interface, GX4_45_MODEL_NUMBER,
               GX4_45_DIAGNOSTICS_STATUS_SEL,
    ...
    while (mip_filter_set_init_attitude(&device_interface, angles) !=
           MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_set_init_heading(&device_interface, angles[0]) !=
           MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_vehicle_dynamics_mode(&device_interface,
                                            MIP_FUNCTION_SELECTOR_LOAD_DEFAULT,
                                            NULL) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_sensor2vehicle_tranformation(
               &device_interface, MIP_FUNCTION_SELECTOR_WRITE, angles) !=
           MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_sensor2vehicle_offset(&device_interface,
                                            MIP_FUNCTION_SELECTOR_WRITE,
                                            offset) != MIP_INTERFACE_OK) {
    }

    // Read back the transformation
    while (mip_filter_sensor2vehicle_offset(
               &device_interface, MIP_FUNCTION_SELECTOR_READ,
               readback_offset) != MIP_INTERFACE_OK) {
    }
    ...
    if ((abs(readback_offset[0] - offset[0]) < 0.001) &&
      ...
      printf("Returned offset: %f X %f Y %f Z\n", readback_offset[0],
             readback_offset[1], readback_offset[2]);
    }
    ...
    while (mip_filter_antenna_offset(&device_interface,
                                     MIP_FUNCTION_SELECTOR_WRITE,
                                     offset) != MIP_INTERFACE_OK) {
    }
    ...
    for (j = 0; j < 0x0020; j++) {
      ...
      // Set control bits
      while (mip_filter_estimation_control(
                 &device_interface, MIP_FUNCTION_SELECTOR_WRITE,
                 &estimation_control) != MIP_INTERFACE_OK) {
      }
    ...
    while (mip_filter_estimation_control(
               &device_interface, MIP_FUNCTION_SELECTOR_LOAD_DEFAULT,
               &estimation_control) != MIP_INTERFACE_OK) {
    }
    ...
    for (i = 2; i >= 1; i--) {
      ...
      while (mip_filter_gps_source(&device_interface,
                                   MIP_FUNCTION_SELECTOR_WRITE,
                                   &gps_source) != MIP_INTERFACE_OK) {
      }
    ...
    printf(
        ...
        "\n");
    printf(
        "Performing External GPS Update with externally supplied GPS "
        "information\n");
    printf(
        "----------------------------------------------------------------------"
        "\n\n");
    ...
    while (mip_filter_gps_source(&device_interface, MIP_FUNCTION_SELECTOR_WRITE,
                                 &gps_source) != MIP_INTERFACE_OK) {
    }
    ...
    external_gps_update.pos[0] =
        44.43753433;  // Latitude of LORD Microstrain Sensing Systems
    external_gps_update.pos[1] =
        -73.10612488;  // Longitude of LORD Microstrain Sensing Systems
    external_gps_update.pos[2] = 134.029999;  // Height Above Ellipsoid of LORD
    ...
    external_gps_update.pos_1sigma[0] =
        0.1;  // North Position Standard Deviation
    external_gps_update.pos_1sigma[1] = 0.1;  // East Position Standard
                                              // Deviation
    external_gps_update.pos_1sigma[2] = 0.1;  // Down Position Standard
                                              // Deviation
    external_gps_update.vel_1sigma[0] =
        0.1;  // North Velocity Standard Deviation
    external_gps_update.vel_1sigma[1] = 0.1;  // East Velocity Standard
                                              // Deviation
    external_gps_update.vel_1sigma[2] = 0.1;  // Down Velocity Standard
                                              // Deviation

    while (mip_filter_external_gps_update(
    ...
    printf(
        ...
        "\n");
    printf(
        "Performing External Heading Update with externally supplied heading "
        "information\n");
    printf(
        "----------------------------------------------------------------------"
        "\n\n");
    ...
    while (mip_filter_heading_source(&device_interface,
                                     MIP_FUNCTION_SELECTOR_WRITE,
                                     &heading_source) != MIP_INTERFACE_OK) {
    }

    external_heading_update.heading_angle = 0.0;  // 0 Radians
    external_heading_update.heading_angle_1sigma =
        0.01;  // Radian Standard Deviation in heading estimate
    external_heading_update.type = 0x1;  // True Heading

    while (mip_filter_external_heading_update(&device_interface,
    ...
    for (i = 3; i >= 0; i--) {
      ...
      mip_filter_heading_source(&device_interface, MIP_FUNCTION_SELECTOR_WRITE,
                                &heading_source);
    ...
    for (i = 1; i >= 0; i--) {
      ...
      while (mip_filter_auto_initialization(&device_interface,
                                            MIP_FUNCTION_SELECTOR_WRITE,
                                            &auto_init) != MIP_INTERFACE_OK) {
      }
    ...
    while (mip_filter_accel_noise(&device_interface,
                                  MIP_FUNCTION_SELECTOR_WRITE,
                                  noise) != MIP_INTERFACE_OK) {
    }

    // Read back the accel noise values
    while (mip_filter_accel_noise(&device_interface, MIP_FUNCTION_SELECTOR_READ,
                                  readback_noise) != MIP_INTERFACE_OK) {
    }
    ...
    if ((abs(readback_noise[0] - noise[0]) < 0.001) &&
      ...
      printf("Returned values: %f X %f Y %f Z\n", readback_noise[0],
             readback_noise[1], readback_noise[2]);
    }
    ...
    while (mip_filter_gyro_noise(&device_interface, MIP_FUNCTION_SELECTOR_WRITE,
                                 noise) != MIP_INTERFACE_OK) {
    }

    // Read back the gyro noise values
    while (mip_filter_gyro_noise(&device_interface, MIP_FUNCTION_SELECTOR_READ,
                                 readback_noise) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_gyro_noise(&device_interface,
                                 MIP_FUNCTION_SELECTOR_LOAD_DEFAULT,
                                 NULL) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_mag_noise(&device_interface, MIP_FUNCTION_SELECTOR_WRITE,
                                noise) != MIP_INTERFACE_OK) {
    }

    // Read back the mag white noise values
    while (mip_filter_mag_noise(&device_interface, MIP_FUNCTION_SELECTOR_READ,
                                readback_noise) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_mag_noise(&device_interface,
                                MIP_FUNCTION_SELECTOR_LOAD_DEFAULT,
                                NULL) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_accel_bias_model(&device_interface,
                                       MIP_FUNCTION_SELECTOR_WRITE, beta,
                                       noise) != MIP_INTERFACE_OK) {
    }

    // Read back the accel bias model values
    while (mip_filter_accel_bias_model(
               &device_interface, MIP_FUNCTION_SELECTOR_READ, readback_beta,
               readback_noise) != MIP_INTERFACE_OK) {
    }
    ...
    if ((abs(readback_noise[0] - noise[0]) < 0.001) &&
      ...
      printf(
          "Sent values:     Beta: %f X %f Y %f Z, White Noise: %f X %f Y %f "
          "Z\n",
          beta[0], beta[1], beta[2], noise[0], noise[1], noise[2]);
      printf(
          "Returned values:  Beta: %f X %f Y %f Z, White Noise: %f X %f Y %f "
          "Z\n",
    ...
    while (mip_filter_accel_bias_model(&device_interface,
                                       MIP_FUNCTION_SELECTOR_LOAD_DEFAULT, NULL,
                                       NULL) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_gyro_bias_model(&device_interface,
                                      MIP_FUNCTION_SELECTOR_WRITE, beta,
                                      noise) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_gyro_bias_model(&device_interface,
                                      MIP_FUNCTION_SELECTOR_READ, readback_beta,
                                      readback_noise) != MIP_INTERFACE_OK) {
    }
    ...
    // Set ZUPT parameters
    while (mip_filter_zero_velocity_update_control(
               &device_interface, MIP_FUNCTION_SELECTOR_WRITE,
    ...
    printf(
        "\n\nResetting Zero Velocity Update Control to default "
        "parameters.\n\n");
    ...
    while (mip_filter_external_heading_update_with_time(
               &device_interface, &external_heading_with_time) !=
           MIP_INTERFACE_OK) {
    }
    ...
    // Set ZUPT parameters
    while (mip_filter_zero_angular_rate_update_control(
               &device_interface, MIP_FUNCTION_SELECTOR_WRITE,
               &zero_update_control) != MIP_INTERFACE_OK) {
    }
    ...
    printf(
        "\n\nResetting Zero Angular Rate Update Control to default "
        "parameters.\n\n");
    ...
    for (i = 1; i < 8; i++) {
      if (mip_filter_tare_orientation(&device_interface,
                                      MIP_FUNCTION_SELECTOR_WRITE,
                                      i) != MIP_INTERFACE_OK) {
    ...
    while (mip_filter_tare_orientation(&device_interface,
                                       MIP_FUNCTION_SELECTOR_LOAD_DEFAULT,
                                       0) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_commanded_zero_velocity_update(&device_interface) !=
           MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_commanded_zero_angular_rate_update(&device_interface) !=
           MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_declination_source(&device_interface,
                                         MIP_FUNCTION_SELECTOR_LOAD_DEFAULT,
                                         NULL) != MIP_INTERFACE_OK) {
    }
    ...
    while (mip_filter_accel_magnitude_error_adaptive_measurement(
               &device_interface, MIP_FUNCTION_SELECTOR_WRITE,
    ...
    if ((accel_magnitude_error_command.enable ==
         accel_magnitude_error_readback.enable) &&
        (abs(accel_magnitude_error_command.low_pass_cutoff -
             accel_magnitude_error_readback.low_pass_cutoff) < 0.001) &&
        (abs(accel_magnitude_error_command.min_1sigma -
             accel_magnitude_error_readback.min_1sigma) < 0.001) &&
        (abs(accel_magnitude_error_command.low_limit -
             accel_magnitude_error_readback.low_limit) < 0.001) &&
        (abs(accel_magnitude_error_command.high_limit -
             accel_magnitude_error_readback.high_limit) < 0.001) &&
        (abs(accel_magnitude_error_command.low_limit_1sigma -
             accel_magnitude_error_readback.low_limit_1sigma) < 0.001) &&
        (abs(accel_magnitude_error_command.high_limit_1sigma -
             accel_magnitude_error_readback.high_limit_1sigma) < 0.001)) {
      printf(
          "accel magnitude error adaptive measurement values successfully "
          "set.\n");
    } else {
      printf(
          "ERROR: Failed to set accel magnitude error adaptive measurement "
          "values!!!\n");
    ...
    printf(
        "\n\nLoading the default accel magnitude error adaptive measurement "
        "values.\n\n");
    ...
    while (mip_filter_mag_magnitude_error_adaptive_measurement(
               &device_interface, MIP_FUNCTION_SELECTOR_WRITE,
    ...
    if ((mag_magnitude_error_command.enable ==
         mag_magnitude_error_readback.enable) &&
        (abs(mag_magnitude_error_command.low_pass_cutoff -
             mag_magnitude_error_readback.low_pass_cutoff) < 0.001) &&
        (abs(mag_magnitude_error_command.min_1sigma -
             mag_magnitude_error_readback.min_1sigma) < 0.001) &&
        (abs(mag_magnitude_error_command.low_limit -
             mag_magnitude_error_readback.low_limit) < 0.001) &&
        (abs(mag_magnitude_error_command.high_limit -
             mag_magnitude_error_readback.high_limit) < 0.001) &&
        (abs(mag_magnitude_error_command.low_limit_1sigma -
             mag_magnitude_error_readback.low_limit_1sigma) < 0.001) &&
        (abs(mag_magnitude_error_command.high_limit_1sigma -
             mag_magnitude_error_readback.high_limit_1sigma) < 0.001)) {
      printf(
          "mag magnitude error adaptive measurement values successfully "
          "set.\n");
    } else {
      printf(
          "ERROR: Failed to set mag magnitude error adaptive measurement "
          "values!!!\n");
      printf("Sent values:     Enable: %i, Parameters: %f %f %f %f %f %f\n",
    ...
    printf(
        "\n\nLoading the default mag magnitude error adaptive measurement "
        "values.\n\n");
    ...
    while (mip_filter_mag_dip_angle_error_adaptive_measurement(
               &device_interface, MIP_FUNCTION_SELECTOR_WRITE,
    ...
    if ((mag_dip_angle_error_command.enable ==
         mag_magnitude_error_readback.enable) &&
        (abs(mag_dip_angle_error_command.low_pass_cutoff -
             mag_dip_angle_error_readback.low_pass_cutoff) < 0.001) &&
        (abs(mag_dip_angle_error_command.min_1sigma -
             mag_dip_angle_error_readback.min_1sigma) < 0.001) &&
        (abs(mag_dip_angle_error_command.high_limit -
             mag_dip_angle_error_readback.high_limit) < 0.001) &&
        (abs(mag_dip_angle_error_command.high_limit_1sigma -
             mag_dip_angle_error_readback.high_limit_1sigma) < 0.001)) {
      printf(
          "mag dip angle error adaptive measurement values successfully "
          "set.\n");
    } else {
      printf(
          "ERROR: Failed to set mag dip angle error adaptive measurement "
          "values!!!\n");
    ...
    if (mip_interface_add_descriptor_set_callback(
            &device_interface, MIP_FILTER_DATA_SET, NULL,
            &filter_packet_callback) != MIP_INTERFACE_OK)
      return -1;

    if (mip_interface_add_descriptor_set_callback(
            &device_interface, MIP_AHRS_DATA_SET, NULL,
            &ahrs_packet_callback) != MIP_INTERFACE_OK)
      return -1;

    if (mip_interface_add_descriptor_set_callback(
            &device_interface, MIP_GPS_DATA_SET, NULL, &gps_packet_callback) !=
        MIP_INTERFACE_OK)
      return -1;
      ...
      while (mip_get_next_field(packet, &field_header, &field_data,
        ...
        switch (field_header->descriptor) {
          ...
          case MIP_FILTER_DATA_NED_VEL: {
            memcpy(&curr_filter_vel, field_data,
...
void print_packet_stats() {
  if (enable_data_stats_output) {
    printf(
        "\r%u FILTER (%u errors)    %u AHRS (%u errors)    %u GPS (%u errors) "
        "Packets",
        filter_valid_packet_count,
...
//
//! @fn
//! u16 mip_3dm_cmd_hw_specific_imu_device_status(mip_interface
//! *device_interface, u16 model_number, u8 status_selector, u8
//! *response_buffer)
//
//! @section DESCRIPTION
...
//! structure.
//! @param [in] u16 model_number - LORD Microstrain Sensing Systems model number
//! for GX4 IMU (6237)
//! @param [in] u8 status selector - specifies which type of status message is
//! being requested.
//! @paran [out] u8 *response_buffer - pointer to the location to store response
//! bytes.
//
//! @retval MIP_INTERFACE_ERROR  Interface not initialized or device not in IMU
//! Direct Mode.\n
...
u16 mip_3dm_cmd_hw_specific_imu_device_status(mip_interface *device_interface,
  ...
  if (status_selector == GX4_IMU_BASIC_STATUS_SEL) {
    if (response_size != sizeof(gx4_imu_basic_status_field))
      ...
      byteswap_inplace(&basic_ptr->device_model,
                       sizeof(basic_ptr->device_model));
      byteswap_inplace(&basic_ptr->status_flags,
                       sizeof(basic_ptr->status_flags));
      byteswap_inplace(&basic_ptr->system_timer_ms,
    ...
    if (response_size != sizeof(gx4_imu_diagnostic_device_status_field))
      ...
      diagnostic_ptr =
          (gx4_imu_diagnostic_device_status_field *)response_buffer;

      byteswap_inplace(&diagnostic_ptr->device_model,
                       sizeof(diagnostic_ptr->device_model));
      byteswap_inplace(&diagnostic_ptr->status_flags,
                       sizeof(diagnostic_ptr->status_flags));
      byteswap_inplace(&diagnostic_ptr->system_timer_ms,
                       sizeof(diagnostic_ptr->system_timer_ms));
      byteswap_inplace(&diagnostic_ptr->gyro_range,
                       sizeof(diagnostic_ptr->gyro_range));
      byteswap_inplace(&diagnostic_ptr->mag_range,
                       sizeof(diagnostic_ptr->mag_range));
      byteswap_inplace(&diagnostic_ptr->pressure_range,
                       sizeof(diagnostic_ptr->pressure_range));
      byteswap_inplace(&diagnostic_ptr->temp_degc,
                       sizeof(diagnostic_ptr->temp_degc));
      byteswap_inplace(&diagnostic_ptr->last_temp_read_ms,
                       sizeof(diagnostic_ptr->last_temp_read_ms));
      byteswap_inplace(&diagnostic_ptr->num_gps_pps_triggers,
                       sizeof(diagnostic_ptr->num_gps_pps_triggers));
      byteswap_inplace(&diagnostic_ptr->last_gps_pps_trigger_ms,
                       sizeof(diagnostic_ptr->last_gps_pps_trigger_ms));
      byteswap_inplace(&diagnostic_ptr->dropped_packets,
                       sizeof(diagnostic_ptr->dropped_packets));
      byteswap_inplace(&diagnostic_ptr->com_port_bytes_written,
                       sizeof(diagnostic_ptr->com_port_bytes_written));
      byteswap_inplace(&diagnostic_ptr->com_port_bytes_read,
                       sizeof(diagnostic_ptr->com_port_bytes_read));
      byteswap_inplace(&diagnostic_ptr->com_port_write_overruns,
                       sizeof(diagnostic_ptr->com_port_write_overruns));
      byteswap_inplace(&diagnostic_ptr->com_port_read_overruns,
...
u16 mip_3dm_cmd_hw_specific_device_status(mip_interface *device_interface,
  ...
  if (status_selector == GX4_45_BASIC_STATUS_SEL) {
    if (response_size != sizeof(gx4_45_basic_status_field))
      ...
      byteswap_inplace(&basic_ptr->status_flags,
                       sizeof(basic_ptr->status_flags));
      byteswap_inplace(&basic_ptr->system_state,
    ...
    if (response_size != sizeof(gx4_45_diagnostic_device_status_field))
      ...
      byteswap_inplace(&diagnostic_ptr->status_flags,
                       sizeof(diagnostic_ptr->status_flags));
      byteswap_inplace(&diagnostic_ptr->system_state,
      ...
      byteswap_inplace(&diagnostic_ptr->last_gps_pps_trigger_ms,
                       sizeof(diagnostic_ptr->last_gps_pps_trigger_ms));
      byteswap_inplace(&diagnostic_ptr->imu_dropped_packets,
                       sizeof(diagnostic_ptr->imu_dropped_packets));
      byteswap_inplace(&diagnostic_ptr->gps_dropped_packets,
                       sizeof(diagnostic_ptr->gps_dropped_packets));
      byteswap_inplace(&diagnostic_ptr->filter_dropped_packets,
                       sizeof(diagnostic_ptr->filter_dropped_packets));
      byteswap_inplace(&diagnostic_ptr->com1_port_bytes_written,
                       sizeof(diagnostic_ptr->com1_port_bytes_written));
      byteswap_inplace(&diagnostic_ptr->com1_port_bytes_read,
                       sizeof(diagnostic_ptr->com1_port_bytes_read));
      byteswap_inplace(&diagnostic_ptr->com1_port_write_overruns,
                       sizeof(diagnostic_ptr->com1_port_write_overruns));
      byteswap_inplace(&diagnostic_ptr->com1_port_read_overruns,
                       sizeof(diagnostic_ptr->com1_port_read_overruns));
      byteswap_inplace(&diagnostic_ptr->imu_parser_errors,
                       sizeof(diagnostic_ptr->imu_parser_errors));
      byteswap_inplace(&diagnostic_ptr->imu_message_count,
                       sizeof(diagnostic_ptr->imu_message_count));
      byteswap_inplace(&diagnostic_ptr->imu_last_message_ms,
                       sizeof(diagnostic_ptr->imu_last_message_ms));
      byteswap_inplace(&diagnostic_ptr->gps_parser_errors,
                       sizeof(diagnostic_ptr->gps_parser_errors));
      byteswap_inplace(&diagnostic_ptr->gps_message_count,
                       sizeof(diagnostic_ptr->gps_message_count));
      byteswap_inplace(&diagnostic_ptr->gps_last_message_ms,
